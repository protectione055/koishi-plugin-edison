...
export async function twitterSearch(
  ...
  const { twitterRestId } = twitter
  try {
    const res = await getTwitterTweets(twitterRestId, ctx, logger)
    
    logger.info(`推特获取成功: ${JSON.stringify(res.tweets)}`)
    if (!res || res.tweets.length === 0)
      return '该用户没有动态。'
    return renderFunction(ctx, res.tweets[0], res.user_info, config, false)
  }
  catch (e) {
